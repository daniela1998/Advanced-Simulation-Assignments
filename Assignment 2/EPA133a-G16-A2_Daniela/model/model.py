...
class BangladeshModel(Model):
    ...
    def __init__(self, seed=None, x_max=500, y_max=500, x_min=0, y_min=0, probabilities = {}, scenario = 0): 
        ...
        for df in df_objects_all:
            for _, row in df.iterrows():    # index, row in ...
                ...
                if model_type == 'source':
                    ...
                    agent = Bridge(row['id'], self, row['length'], row['name'], row['road'], row['condition'], self.probabilities, self.scenario)
    ...
    # To collect and export to csv
    def collect_truck_data(self):
        truck_data = []
        for agent in self.schedule.agents:
            # How do I change this to be able to access Vehicle attributes from Source class?
            if isinstance(agent,Source):
                truck_data.append({
                    agent.generate_truck().agent.waiting_time,
                    agent.generate_truck().agent.unique_id,
