...
scenario_number_range =range(1,9) #modifed


for n, seed in enumerate(scenario_number_range): #changed

    sim_model = BangladeshModel(seed=seed, probabilities = scenario_probabilities, scenario =n)   #modified
    ...
    df.to_csv(f'scenario{n}_summary.csv', index=False, sep=';')


    # Save the collected data ONLY from Vehicles to a CSV file
    sim_model.save_data(f'Vehicles-scenario{n}-seed {seed}.csv') #modified
