...
class BangladeshModel(Model):
    ...
    def generate_model(self):
        ...
        df = pd.read_csv('../data/demo-100.csv')
        ...
        for df in df_objects_all:
            for _, row in df.iterrows():    # index, row in ...
                ...
                if model_type == 'source':
                    ...
                    agent = Bridge(row['id'], self, row['length'], row['name'], row['road'], row['condition']) #modified
    ...
    def collect_data(self): #modified
        """
        ...
        for agent in self.schedule.agents:
            if isinstance(agent, Vehicle):
                self.data.append({
                    ...
                    'Accumulated waiting time':agent.accumulated_waiting_time,
                    'Total driving time': agent.total_driving_time #it is only completed when removed :(
                })

            if isinstance(agent, Bridge) and agent.vehicle_count>0:
                self.data.append({
                    'step': self.schedule.steps,
                    'model type': agent.__class__.__name__,
                    'id': agent.unique_id,
                    'location': '-',
                    'location_offset': '-',
                    'state': agent.condition,
                    'waiting_time': agent.delay_time,
                    'generated_at_step': '-',
                    'removed_at_step':'-',
                    'Accumulated waiting time':'-',
                    'Total driving time': '-'
                })#it is only completed when removed :(
    ...
    def save_data(self, filename='scenario0.csv'):
        """
